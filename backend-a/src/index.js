...
async function pollQueueA() {
  if (!QUEUE_A_URL) {
    console.warn('QUEUE_A_URL not set, skipping SQS consumer');
    return;
  }
  const receive = async () => {
    try {
      ...
      if (Messages && Messages.length > 0) {
        for (const msg of Messages) {
          ...
          console.log('Processed message from queue-a:', msg.MessageId);
          if (QUEUE_B_URL) {
            await sqs.send(new SendMessageCommand({
              QueueUrl: QUEUE_B_URL,
              MessageBody: JSON.stringify({ from: 'backend-a', originalId: msg.MessageId, at: receivedAt }),
            }));
          }
        }
      }
...
app.post('/send', async (req, res) => {
  if (!QUEUE_B_URL) return res.status(500).json({ error: 'QUEUE_B_URL not set' });
  const body = req.body?.body ?? req.body ?? 'hello from backend-a';
  try {
